...
class RandomPE(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.max_len = 10752  # different from windows
...
class NeuronPE(nn.Module):
    def __init__(
        ...
        # 定义固定的正弦余弦基
        pe_base = torch.zeros(self.max_len, self.num_pe_neuron)
        ...
        pe_base[:, 0::2] = torch.sin(position * div_term)
        pe_base[:, 1::2] = torch.cos(position * div_term_single)

        pe_base = pe_base.unsqueeze(0).transpose(0, 1)  # MaxL, 1, Neur
        self.register_buffer("pe_base", pe_base)

        # 新增：定义每个位置神经元通道的门控参数
        self.gate = nn.Parameter(torch.randn(1, 1, self.num_pe_neuron))  # 1,1,Neur
    ...
    def forward(self, x):
        ...
        # 根据 gate 决定激活哪些位置编码
        pe = self.pe_base[:x.size(-2), :]  # TL, 1, Neur
        gate_score = torch.sigmoid(self.gate)  # 1, 1, Neur
        gated_pe = pe * gate_score  # TL, 1, Neur

        if self.pe_mode == "concat":
            tmp = gated_pe.repeat(1, B, 1).transpose(0, 1)  # B, TL, Neur
            x = torch.cat([x, tmp], dim=-1)
        elif self.pe_mode == "add":
            x = x + gated_pe.transpose(0, 1)  # [B, TL, Neur]
