...
# def robust_batch_corr(y_true, y_pred):
#     """
#     计算 y_true 和 y_pred 之间的皮尔逊相关系数，使用手动方式（包括均值和标准差）。
#
#     参数:
#         y_true: 真实值（numpy 数组）
#         y_pred: 预测值（numpy 数组）
...
#
#     # 计算皮尔逊相关系数
#     correlation = np.mean((y_pred - mean_pred) * (y_true - mean_true)) / (sigma_pred * sigma_true)
#
#     return correlation
...
#     """
#     Calculate the Root Mean Square Error (RMSE) between predictions and ground truth.自己家的rmse
#
#     Args:
#         y_true: Ground truth values (numpy array or PyTorch tensor)
#         y_pred: Predicted values (numpy array or PyTorch tensor)
#
#     Returns:
#         RMSE value as float (scalar)
#     """
#     if isinstance(y_true, np.ndarray):
#         # For numpy arrays
#         loss = (y_true - y_pred) ** 2
#         return np.sqrt(np.nanmean(loss))
#
#     # For PyTorch tensors
#     mask = ~torch.isnan(y_true)
#     y_pred = torch.masked_select(y_pred, mask)
...
def get_metric_fn(eval_metric):
    ...
    if eval_metric == "corr":
        ...
        return lambda y_true, y_pred: np.corrcoef(y_true.flatten(), y_pred.flatten())[0, 1]
    if eval_metric == "mse":
        return single_mse
    if eval_metric == "mae":
        return lambda y_true, y_pred: np.mean(np.abs(y_true.flatten() - y_pred.flatten()))
    if eval_metric == "rmse":
        return lambda y_true, y_pred: np.sqrt(np.mean((y_true.flatten() - y_pred.flatten()) ** 2))
    if eval_metric == "mape":
        return lambda y_true, y_pred: (
            np.mean(np.abs(y_true.flatten()[y_true.flatten() != 0] - y_pred.flatten()[y_true.flatten() != 0]) /
                    y_true.flatten()[y_true.flatten() != 0] * 100)
            if np.any(y_true.flatten() != 0) else np.nan
        )
